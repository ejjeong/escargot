...
{
    // XXX it assumes that c is lower than 0xD800. 
    uint32_t props = propsTrie_index[(propsTrie_index[0 + (c >> 5)] << 2) + (c & 0x1F)];
...
ALWAYS_INLINE bool isIdentifierStart(char16_t ch)
{
    ...
    return (ch >= 97 && ch <= 122) // a..z
        ...
        || (ch == 92) // \ (backslash)
        // || (U_GET_GC_MASK(c) & (1 << 1 | 1 << 2 | 1 << 3 | 1 << 4 | 1 << 5));
        || ((1 << u_charType(ch)) & 0x3E); // see icu's source/common/unicode/uchar.[c|h]
}
...
ALWAYS_INLINE bool isIdentifierPart(char16_t ch)
{
    ...
    return (ch >= 97 && ch <= 122) // a..z
        ...
        || (ch == 36) || (ch == 95) // $ (dollar) and _ (underscore)
        || (ch == 92) // \ (backslash)
        || ((1 << u_charType(ch)) & (0x3E | 1 << 6 | 1 << 8 | 1 << 9 | 1 << 22)) || ch == 0x200C || ch == 0x200D;
}

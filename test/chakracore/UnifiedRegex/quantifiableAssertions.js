...
var numGreedyQuantifiers = quantifiers.length;
for (var q = 0; q < numGreedyQuantifiers; ++q)
    quantifiers.push(quantifiers[q] + "?");
var regexFunctions = [
    { f: exec, caresAboutGlobal: true },
    { f: test, caresAboutGlobal: true },
    { f: match, caresAboutGlobal: true },
    { f: replace, caresAboutGlobal: true },
    { f: split, caresAboutGlobal: false },
    { f: search, caresAboutGlobal: false }
];
for (var a = 0; a < assertions.length; ++a) {
    for (var q = 0; q < quantifiers.length; ++q) {
        for (var rf = 0; rf < regexFunctions.length; ++rf) {
            var regexes = ["/" + assertions[a] + quantifiers[q] + "/"];
            if (regexFunctions[rf].caresAboutGlobal)
                regexes.push(regexes[0] + "g");
            for (var r = 0; r < regexes.length; ++r) {
                try {
                    var args = [eval(regexes[r]), "aaaaa"];
                    if (regexFunctions[rf].f === replace)
                        args.push("x");
                    regexFunctions[rf].f.apply(this, args);
                } catch (ex) {
                    echo(regexes[r]);
                    // echo(ex);
                    echo("EXCEPTION");
